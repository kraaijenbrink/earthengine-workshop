...
author: "[Dr. Philip Kraaijenbrink](https://www.uu.nl/staff/PDAKraaijenbrink)"
...
*This content has been developed originally for an internal workshop at the Department of Physical Geography of Utrecht University.*
...
To use earth engine you need to [sign up](https://earthengine.google.com/new_signup/) for the service first using a Google account.

A script repository that holds working code of all the exercises as well as additional cases can be added to your own Earth Engine environment using the following URL:  
<https://code.earthengine.google.com/?accept_repo=users/philipkraaijenbrink/dpg-workshop>

You are also advised to add the open repository *Philip's tools* to your Earth Engine environment. This repository contains additional functions that are called by the scripts in this manual. By adding the repository to your environment, you can see the code and review its functionality:  
<https://code.earthengine.google.com/?accept_repo=users/philipkraaijenbrink/tools>

&nbsp;

**Remember that you can search for help on functions in the docs tab on the top left panel, and that there is a concise [online user guide](https://developers.google.com/earth-engine/) with clear examples, accessible via the help dropdown menu in the top right corner.**
...
**Generate a Landsat 8 composite image, perform a landcover classification of the image, and compare class distribution over elevation in the Netherlands.**
...
**Perform a regression analysis on MODIS NDVI to observe the spatial distribution of greening and drying over the last two decades. The NDVI stands for the [Normalized Difference Vegetation Index](https://earthobservatory.nasa.gov/Features/MeasuringVegetation/measuring_vegetation_2.php) (-1: bare, to 1: highly vegetated), which is a measure for how much and how healthy vegetation is present in a pixel** 
...
The NEX-GDDP dataset includes downscaled projections for [RCP4.5 and RCP8.5](https://en.wikipedia.org/wiki/Representative_Concentration_Pathways) from 21 Global Circulation Models and scenarios for which daily scenarios were produced and distributed under [CMIP5](https://en.wikipedia.org/wiki/Coupled_model_intercomparison_project). Each of the climate projections includes daily maximum temperature, minimum temperature, and precipitation for the periods from 1950 through 2099. The spatial resolution of the dataset is 0.25 degrees (approximately 27 by 27 km). The original NEX-GDDP data is present in the Earth Engine catalog. For the purpose of this exercise I have pre-aggregated it to yearly data and calculated model ensemble statistics using Earth Engine's computing power.


Load the aggregated NEX-GDDP data, MODIS water mask and custom code:
